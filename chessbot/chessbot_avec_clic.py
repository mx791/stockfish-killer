...
from PyQt5.QtWidgets import QApplication, QWidget, QPushButton, QLabel, QStylePainter
...
def minimax(chessboard, depth, player, alpha, beta):
    ...
    for move in legal_moves :
        ...
        if chessboard.is_check() :     
            [move, score] = minimax(chessboard, min(12,depth), -player, alpha, beta)
...
class MainWindow(QWidget):
    ...
    def game_ending(self): #detection des mats et pats
    ...
    def play_strategy(self): # Ici, stratégie minimax, depth = profondeur de la recherche des coups
        ...
        move = minimax(self.chessboard, depth= 4, player=1, alpha= -np.inf, beta = np.inf)
        move = move[0]
        print(move)
                ...
                if move in self.legal_moves :
                    ...
                    if self.chessboard.piece_at(promotion_squares[int(click_list[0][1])]).piece_type == 1 : # gros bordel pour les pions qui arrivent à la promotion
                        if (move + 'q') in self.legal_moves :
                            ...
                            painter.setPen(QPen(QColor(1,225,0,225),  5))  
                            painter.drawRect(10, 400, 400, 200)
                            self.chessboardSvg = chess.svg.board(self.chessboard).encode("UTF-8")
